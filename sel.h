void createLocalA(Matrix &A,mesh m){
    float pt = m.getParameter(P_T);
    A.at(0).at(0) += -pt/8;  A.at(0).at(1) += pt/8;
    A.at(1).at(0) += -pt/8;  A.at(1).at(1) += pt/8;
}

void createLocalB(Matrix &B,mesh m){
    float l = m.getParameter(ELEMENT_LENGTH);
    float pk = m.getParameter(P_K);
    B.at(0).at(0) += pk/l;      B.at(0).at(1) += -pk/l;
    B.at(1).at(0) += -pk/l;     B.at(1).at(1) += pk/l;
}

void createLocalC(Matrix &C,mesh m){
    float py = m.getParameter(P_Y);
    C.at(0).at(0) += -py/3;    C.at(0).at(1) += py/3;
    C.at(1).at(0) += -py/3;    C.at(1).at(1) += py/3;
}

void createLocalD(Matrix &D,mesh m){
    float pv = m.getParameter(P_V);
    float l = m.getParameter(ELEMENT_LENGTH);
    D.at(0).at(0) += pv/l;  D.at(0).at(1) += -pv/l;
    D.at(1).at(0) += -pv/l;  D.at(1).at(1) += pv/l;
}

void createLocalE(Matrix &E,mesh m){
    float pc = m.getParameter(P_C);
    E.at(0).at(0) += -(3*pc)/2;    E.at(0).at(1) += (3*pc)/2;
    E.at(1).at(0) += -(3*pc)/2;    E.at(1).at(1) += (3*pc)/2;
...
void createLocalF(Matrix &F,mesh m){
    ...
    float ps = m.getParameter(P_S);
    F.at(0).at(0) += -ps/2;    F.at(0).at(1) += ps/2;
    F.at(1).at(0) += -ps/2;    F.at(1).at(1) += ps/2;
}
...
Vector createLocalb(int element,mesh &m){
    ...
    float pw = m.getParameter(P_W), pn = m.getParameter(P_N), l = m.getParameter(ELEMENT_LENGTH);
